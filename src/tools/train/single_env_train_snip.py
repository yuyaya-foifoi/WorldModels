...
from src.model.init_utils import weights_init
...
config.dp = '../data'
masks = None
mb = ModelBase(config.network, config.depth, config.dataset, value_model)
mb.cuda()
trainloader, testloader = get_dataloader(config.dataset, config.batch_size, 256, 0, root=config.dp)

# ===== pruning =====
from src.pruner.pruning import *
config.rank_algo ='snip' #'rank algorithm (choose one of: snip, grasp, synflow)'
#config.prune_mode = 'panning' #'prune mode (choose one of: dense, rank, rank/random, rank/iterative, fcs, panning)'
config.prune_mode = 'dense'
mb.model.apply(weights_init)
masks, _ = Pruner(mb, trainloader, 'cuda', config)
mb.register_mask(masks)
#pr_str, rem_ratio, eff_ratio = masks_compare(mb, masks, trainloader, 'cuda')#このコードは必須じゃない
...
if __name__ == "__main__":
    log_dir = os.path.join(
        ...
        CONFIG_DICT["experiment"]["env_name"] + "_" + get_str_currentdate(),
    )
    os.makedirs(log_dir, exist_ok=True)
    shutil_copy("./configs/single_env_config.py", log_dir)
    main()
