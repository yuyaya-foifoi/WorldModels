...
from src.ftl import get_FTL_weight
...
if transfer_type == "fractional":
    ...
    FTL_encoder_weight = get_FTL_weight(
        model=encoder.to(device),
        log_dir=log_dir,
        pth_name="encoder.pth",
    )

    encoder.load_state_dict(FTL_encoder_weight)

    FTL_action_model_weight = get_FTL_weight(
    ...
    action_model.load_state_dict(FTL_action_model_weight)

    FTL_value_model_weight = get_FTL_weight(
    ...
    value_model.load_state_dict(FTL_value_model_weight)

    FTL_reward_model_weight = get_FTL_weight(
    ...
    rssm.reward.load_state_dict(FTL_reward_model_weight)

    FTL_transition_model_weight = get_FTL_weight(
        model=rssm.transition.to(device),
        log_dir=log_dir,
        pth_name="rssm.pth",
    )

    rssm.transition.load_state_dict(FTL_transition_model_weight)

    FTL_observation_model_weight = get_FTL_weight(
        model=rssm.observation.to(device),
        log_dir=log_dir,
        pth_name="obs_model.pth",
    )

    rssm.observation.load_state_dict(FTL_observation_model_weight)
    ...
    value_fc4_init_weight = copy.deepcopy(value_model.fc4.weight)
    value_fc4_init_bias = copy.deepcopy(value_model.fc4.bias)
    ...
    reward_fc4_init_weight = copy.deepcopy(rssm.reward.fc4.weight)
    reward_fc4_init_bias = copy.deepcopy(rssm.reward.fc4.bias)
    ...
    rssm.transition.load_state_dict(torch.load(os.path.join(log_dir, "rssm.pth")))
    rssm.observation.load_state_dict(torch.load(os.path.join(log_dir, "obs_model.pth")))
...
if __name__ == "__main__":
    ...
    shutil_copy(
        "./configs/without_prune/transfer_single_env_config.py", log_dir
    )
    main()
